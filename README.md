...
* `id` - первичный ключ, идентификатор вакансии в API Worki
...
Модель `Profession` служит для хранения справочника вакансий в API Worki и имеет следующие атрибуты:

* `id` - первичный ключ, идентификатор профессии в API Worki
...
* `id` - первичный ключ, идентификатор отклика в API Worki
...
* `last_online_time` - необязательный атрибут, время последнего входа в приложение Worki
...
* `active_candidate` - обязательный атрибут, флог, сигнализирующий о том, что кандидат действительно рассматривает предложения и активен в системе
